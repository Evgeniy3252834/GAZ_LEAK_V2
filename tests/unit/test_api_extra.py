"""
Дополнительные тесты для API для достижения 70% покрытия
"""
import pytest
...
class TestAPIExtra:
    ...
    def test_process_video_with_real_path(self, mock_path, mock_usecase):
        """Тест обработки видео с реальным путем"""
    ...
    def test_batch_process_with_callback(self, mock_path, mock_processor):
        """Тест батчевой обработки с callback URL"""
        mock_path_instance = MagicMock()
        mock_path_instance.exists.return_value = True
        mock_path.return_value = mock_path_instance
